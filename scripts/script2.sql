...
2 CTES JOINED TOGETHER IN A WAY THAT INVOLVES ONLY GUYS THAT ARE IN BOTH
INNER JOIN 
...
SELECT p.playerid, SUM(p.HR) AS total_homeruns, DATE_PART('year', CAST(l.debut AS date)) AS decade
FROM pitchingpost AS p
LEFT JOIN people AS l
ON p.playerid = l.playerid
WHERE DATE_PART('year', CAST(l.debut AS date)) < 2006
GROUP BY p.playerid, l.debut;